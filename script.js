(async function() {
    ...
    regCapturar1.addEventListener('click', async () => {
    ...
    regCapturar2.addEventListener('click', async () => {
        const deteccion = await obtenerDescriptor(regVideo);
        if (!deteccion) {
            regEstado.innerText = '❌ No se detectó rostro. Intente de nuevo.';
            return;
    ...
    document.getElementById('regCancelar').addEventListener('click', cerrarRegistro);
    ...
    document.getElementById('btnFichar').addEventListener('click', async () => {
        modalFichar.classList.add('activo');
        await iniciarCamaraFichar();
    });

    document.getElementById('ficharCerrar').addEventListener('click', cerrarFichar);

    // ---------- ADMIN: LISTA Y DETALLE (con nuevo diseño de extras) ----------
    ...
    function calcularExtras(employeeId) {
        ...
        for (let diaStr in horasPorDia) {
            const horas = horasPorDia[diaStr] / 60;
    ...
    // Eventos admin
    document.getElementById('btnVerFichas').addEventListener('click', () => {
    ...
    document.getElementById('btnAccederAdmin').addEventListener('click', () => {
    ...
    document.getElementById('verCerrar').addEventListener('click', () => {
